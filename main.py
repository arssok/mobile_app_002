"""
Исправлен баг- после перезапуска скорость не сбрасывалась на начальную (Испр.65 стр)
В начале игры змейка появляется в центре экрана в квадрате 10 на 6
Добавлен в менеджер экран Победы с кнопкой продолжения игры и условие победы по числу съеденных яблок
Добавил, что как только змейка съела фрукт, и сразу появлялся новый без ожидания таймера
Настроил размещение виджетов в части kv
"""
import kivy

from kivy.app import App
from kivy.uix.widget import Widget
from kivy.properties import ObjectProperty,NumericProperty,ListProperty,BooleanProperty,OptionProperty,ReferenceListProperty,StringProperty
...
class Playground(Widget):
    ...
    def update(self, *args):
        ...
        if self.is_defeated():
            ...
            sound.play()
            Snake3App.screen_manager.current = "welcome_screen"
            return
        ...
        if self.fruit.is_on_board():
            if self.snake.get_position() == self.fruit.pos:
                ...
                if self.score > 9:
                    ...
                    sound.play()
                    self.reset()
                    Snake3App.screen_manager.current = "victory_screen"
                    return
...
class Snake3App(App):
    ...
    def build(self):
        ...
        Snake3App.screen_manager = ScreenManager()
